(ns site-prep.operation
  "OperationActor -- one site-preparation-project coordination operation =
  one supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Site Prep Advisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Site Prep Governor (:govern) and
  ...
  One graph run = one site-preparation-coordination operation (intake ->
  ...
  decision to a human operator (the licensed engineer / site supervisor).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:schedule-site-operation`/`:flag-safety-concern` ALWAYS
  reach this node -- see `site-prep.phase`/`site-prep.governor` ns
  docstrings. `:log-site-record` and `:order-supplies` (below the cost
  threshold) MAY auto-commit at phase 3 when the governor is clean -- see
  `site-prep.phase` ns docstring.

  Every committed record carries `:effect :propose` (see `site-prep.
  ...
  ADDITIONALLY send the safety-concern notice (mail + phone, `site-prep.
...
(defn build
  ...
  `site-prep.store/Store`).
  opts:
    :advisor      -- a `site-prep.advisor/Advisor` (default: mock-advisor)
    :notifier     -- a `site-prep.notify/Notifier` (default: mock-notifier)
  ...
  (-> (g/state-graph
      ...
      ;; Site Prep Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Site Prep Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
