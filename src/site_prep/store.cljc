(ns site-prep.store
  "SSoT for the site-preparation-coordination actor, behind a `Store`
  ...
  (test/site_prep/store_contract_test.clj), which is the whole point: the
  actor, the Site Prep Governor and the audit ledger never know which SSoT
  they run on.
  ...
  rather than the hand-rolled pattern earlier actors (incl.
  `demolition.store`) predate.
  ...
  counter: `:log-site-record` (site-record-log), `:schedule-site-
  ...
  never actually dispatches/authorizes/files/hands-over anything -- it
  only ever proposes, logs and schedules (`:effect :propose`
  unconditionally, see `site-prep.governor` ns docstring). The site's own
  `:site-verified?` / `:utility-locate-completed?` / `:safety-concern-
  unresolved?` / `:notification-lead-days-actual` ground-truth fields are
  what the Site Prep Governor independently re-checks before a
  `:schedule-site-operation` proposal may ever commit (see `site-prep.
  governor`).
  ...
  (:require [site-prep.registry :as registry]
...
(defprotocol Store
  ...
  (all-sites [s])
  (ledger [s])
  (site-record-log-history [s] "the append-only site-record-log history (site-prep.registry drafts)")
  (schedule-proposal-history [s] "the append-only site-operation schedule-proposal history")
...
(defn demo-data
  ...
  []
  {:sites
   {"site-1" {:id "site-1" :name "Sakura New Residential Lot -- Site Preparation"
              :jurisdiction "JPN" :site-verified? true :utility-locate-completed? true
              :contamination-detected? false :safety-concern-unresolved? false
              :notification-lead-days-actual 10
              :safety-contacts [{:name "Tanaka (site supervisor)" :email "tanaka@example.com" :phone "+819000000001"}
                                {:name "Suzuki (licensed engineer)" :email "suzuki@example.com" :phone "+819000000002"}]
              :status :ready-to-schedule}
    "site-2" {:id "site-2" :name "Atlantis Industrial Park Lot -- Site Preparation"
              :jurisdiction "ATL" :site-verified? true :utility-locate-completed? true
              :contamination-detected? false :safety-concern-unresolved? false
              :notification-lead-days-actual 10
              :safety-contacts []
              :status :intake}
    "site-3" {:id "site-3" :name "鈴木団地 造成工事"
              :jurisdiction "JPN" :site-verified? false :utility-locate-completed? false
              :contamination-detected? false :safety-concern-unresolved? false
              :notification-lead-days-actual nil
              :safety-contacts [{:name "Sato (site supervisor)" :email "sato@example.com" :phone "+819000000003"}]
              :status :unverified}
    "site-4" {:id "site-4" :name "田中倉庫用地 造成工事"
              :jurisdiction "JPN" :site-verified? true :utility-locate-completed? false
              :contamination-detected? nil :safety-concern-unresolved? false
              :notification-lead-days-actual 10
              :safety-contacts [{:name "Ito (site supervisor)" :email "ito@example.com" :phone "+819000000004"}]
              :status :utility-locate-pending}
    "site-5" {:id "site-5" :name "Liberty Ave Lot -- Site Preparation"
              :jurisdiction "JPN" :site-verified? true :utility-locate-completed? true
              :contamination-detected? true :safety-concern-unresolved? false
              :notification-lead-days-actual 2
              :safety-contacts [{:name "Jordan (site supervisor)" :email "jordan@example.com" :phone "+15550000001"}]
              :status :notification-pending}
    "site-6" {:id "site-6" :name "ことぶき第二公園予定地 造成工事"
              :jurisdiction "JPN" :site-verified? true :utility-locate-completed? true
              :contamination-detected? true :safety-concern-unresolved? true
              :notification-lead-days-actual 10
              :safety-contacts [{:name "Watanabe (site supervisor)" :email "watanabe@example.com" :phone "+819000000005"}
                                {:name "Yamamoto (licensed engineer)" :email "yamamoto@example.com" :phone "+819000000006"}]
              :status :concern-open}
    "site-7" {:id "site-7" :name "Riverside Distribution Center Lot -- Site Preparation"
              :jurisdiction "USA" :site-verified? true :utility-locate-completed? true
              :contamination-detected? false :safety-concern-unresolved? false
              :notification-lead-days-actual 3
              :safety-contacts [{:name "Casey (site supervisor)" :email "casey@example.com" :phone "+15550000002"}]
              :status :ready-to-schedule}
    "site-8" {:id "site-8" :name "Alt-Gewerbegebiet Baufeld -- Erdarbeiten"
  ...
  (commit-record! [s {:keys [op path value]}]
    (let [site-id (first path)]
      (case op
        ...
        (let [{:keys [result]} (schedule-site-operation! s site-id)
...
(def ^:private site-spec
  ...
  {:id                             {:attr :site/id}
   ...
   :contamination-detected?        {:attr :site/contamination-detected? :coerce boolean}
   :safety-concern-unresolved?     {:attr :site/safety-concern-unresolved? :coerce boolean}
   :notification-lead-days-actual  {:attr :site/notification-lead-days-actual}
