(ns site-prep.store-contract-test
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      (is (= "Sakura New Residential Lot -- Site Preparation" (:name (store/site s "site-1"))))
      (is (= "JPN" (:jurisdiction (store/site s "site-1"))))
      (is (true? (:site-verified? (store/site s "site-1"))))
      (is (true? (:utility-locate-completed? (store/site s "site-1"))))
      (is (false? (:safety-concern-unresolved? (store/site s "site-1"))))
      (is (= 10 (:notification-lead-days-actual (store/site s "site-1"))))
      (is (false? (:site-verified? (store/site s "site-3"))) "site-3 seeded not-yet-verified")
      (is (false? (:utility-locate-completed? (store/site s "site-4"))) "site-4 seeded with an incomplete utility-locate survey")
      (is (= 2 (:notification-lead-days-actual (store/site s "site-5"))) "site-5 seeded with an insufficient JPN lead time")
    ...
    (testing label
      (testing "log-site-record commits a patch and appends a site-record-log entry"
        (store/commit-record! s {:op :log-site-record :path ["site-1"]
                                 :value {:id "site-1" :contamination-detected? true}})
        (is (true? (:contamination-detected? (store/site s "site-1"))))
        (is (= "Sakura New Residential Lot -- Site Preparation" (:name (store/site s "site-1"))) "unrelated field preserved")
        (is (= "JPN-SRL-000000" (get (first (store/site-record-log-history s)) "record_id")))
        (is (= 1 (store/next-site-record-sequence s "JPN"))))
      (testing "schedule-site-operation drafts a record and advances the sequence"
        (store/commit-record! s {:op :schedule-site-operation :path ["site-1"]
      ...
      (testing "flag-safety-concern sets safety-concern-unresolved? true, drafts a record + notice document"
        (store/commit-record! s {:op :flag-safety-concern :path ["site-1"]
                                 :value {:site-id "site-1" :concern-description "test buried line"}})
      ...
      (testing "order-supplies drafts a record and advances the sequence"
        (store/commit-record! s {:op :order-supplies :path ["site-1"]
                                 :value {:site-id "site-1" :items ["bin"] :cost-usd 500}})
      ...
      (testing "sequences are jurisdiction-scoped, not global"
        (store/commit-record! s {:op :log-site-record :path ["site-7"]
                                 :value {:id "site-7" :contamination-detected? true}})
...
(deftest datomic-empty-store-is-usable
  (let [s (store/datomic-store)]
    ...
    (store/with-sites s {"x" {:id "x" :name "n" :jurisdiction "JPN"
                              :site-verified? true :utility-locate-completed? true
                              :safety-concern-unresolved? false :notification-lead-days-actual 10
                              :status :intake}})
