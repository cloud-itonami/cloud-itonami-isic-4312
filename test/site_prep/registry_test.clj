(ns site-prep.registry-test
  (:require [clojure.test :refer [deftest is]]
            [site-prep.registry :as r]))
...
(def sample-site
  {:id "site-1" :name "Sakura New Residential Lot -- Site Preparation" :jurisdiction "JPN"})

(deftest safety-concern-notice-cites-legal-basis-inline
  (let [doc (r/render-safety-concern-notice sample-site "JPN-SCF-000000" "掘削中に未記載の埋設物を確認")]
    (is (re-find #"JPN-SCF-000000" doc))
    (is (re-find #"労働安全衛生規則" doc))
    (is (re-find #"laws\.e-gov\.go\.jp" doc))
    (is (re-find #"未記載の埋設物" doc))
    (is (re-find #"licensed engineer" doc) "honestly disclaims site-readiness sign-off authority")))
